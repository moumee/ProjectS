...
// Sets default values
ASuraPlayerMovementTrigger::ASuraPlayerMovementTrigger()
{
	...
	if (IPlayerInterface* PlayerInterface = Cast<IPlayerInterface>(OtherActor))
	{
		switch (MovementTriggerType)
		{
			case EMovementTriggerType::Start:
				{
					if (!PlayerMovementDataModifiers.IsEmpty())
						PlayerInterface->RequestMovementDataModification(PlayerMovementDataModifiers);

					if (!PlayerKeyHoldModifiers.IsEmpty())
						PlayerInterface->RequestMovementKeyHoldModification(PlayerKeyHoldModifiers);
				}
				break;
			case EMovementTriggerType::End:
				{
					PlayerInterface->RequestResetModification();
				}
				break;
			default:
				break;
